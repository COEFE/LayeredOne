...
# Script to set up PDF support for the application
echo "Setting up PDF support for the LLM Chat App..."

# Install pdf-parse and update react-pdf
echo "Installing PDF-related dependencies..."
npm install pdf-parse react-pdf@7.1.2 --legacy-peer-deps

# Check if the dependencies were installed successfully
if [ $? -eq 0 ]; then
  echo "✅ PDF dependencies installed successfully."
else
  echo "❌ Failed to install PDF dependencies. Please check your package manager."
  exit 1
fi

# Create pdf extractor file if it doesn't exist
if [ ! -f "./src/utils/pdfExtractor.ts" ]; then
  echo "Creating PDF extractor utility..."
  cat > ./src/utils/pdfExtractor.ts << 'EOF'
/**
...
EOF
  echo "✅ Created PDF extractor utility."
else
  echo "PDF extractor utility already exists."
fi

echo "PDF support setup completed!"
echo "You can now process and view PDF documents in the application."